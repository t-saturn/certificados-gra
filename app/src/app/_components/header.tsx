    ...
    <header className="fixed top-0 left-0 right-0 z-50 bg-background/80 backdrop-blur-md border-b border-border">
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div className="flex h-20 items-center justify-between">
          ...
          <div className="hidden items-center gap-3 lg:flex">
            <ThemeToggle />
            <Link href="/login" className="px-6 py-2.5 text-sm font-semibold bg-primary text-primary-foreground rounded-full transition-all duration-200 hover:bg-primary/90 hover:shadow-lg active:scale-95">
              Iniciar Sesión
            </Link>
          </div>
      ...
      <div className={`fixed inset-0 top-20 z-40 lg:hidden bg-background transition-all duration-300 ${isMobileMenuOpen ? 'opacity-100 visible' : 'opacity-0 invisible pointer-events-none'}`}>
        <nav className="flex flex-col gap-2 p-6">
          ...
          <Link href="/login" className="mt-4 px-6 py-3 text-center text-base font-semibold bg-primary text-primary-foreground rounded-full transition-all duration-200 hover:bg-primary/90">
            Iniciar Sesión
          </Link>
        </nav>
      </div>
